...
| 🧵 多步任务 | "坑底挖矿，然后回地面拿绳子" |
| 🌲 砍树 | "砍点木头" "收集木材"（自动选斧头） |
| 🎣 钓鱼 | "钓鱼" "钓几条鱼"（感知生物群系选水域，自动消耗鱼饵） |
| 🚶 跟随/停止 | "跟着我" "别跟了" "停下" |
| 🎁 物品交互 | "把铁锭给我" "收进箱子" "铁锭怎么合成" |
| 🧭 传送 | "传送到 (x,y)" |
| ⛏️ 探索 | "去地下看看"（完整地下探险：找洞口→下挖→挖矿→回家） |
| ♾️ 长期任务 | "一直挖铁" "守在这里" |
| 🧠 记忆 | "记住我喜欢用铁镐" "我之前说过什么" |

> 猫娘会**干着聊着**：挖矿念叨进度、战斗惊呼、危险解除后吐槽衔接、主人静默久了主动关心。
> 她**什么任务用什么工具**：挖矿掏镐、砍树拿斧、钓鱼甩竿、打架自动切伤害最高的武器（近战/远程/魔法都会用）。
...
| 域 | 能力 | 工具/引擎 |
|----|------|-----------|
| 采集 | 挖矿/砍树/钓鱼，背包增量计数 | `terraria_mine` · `MiningEngine`/`LifeEngine` |
| 战斗 | 风筝/黑名单/低血保命/自动选武器 | `CombatEngine` |
| 合成 | mod 真实配方推演，缺材料提示 | `terraria_recipe`/`craft` · `RecipeBook` |
| 工具 | 什么任务用什么工具（镐/斧/钓竿/武器） | `LifeEngine.select_tool` |
| 钓鱼 | 生物群系感知选水域，自动消耗鱼饵 | `LifeEngine.fish` |
| 基地 | 出生点定位/储物/魔镜回家/背包满补给 | `BaseManager` |
| 探索 | 地下探险闭环（找洞→下挖→挖矿→回家） | `UndergroundExplorer` |
